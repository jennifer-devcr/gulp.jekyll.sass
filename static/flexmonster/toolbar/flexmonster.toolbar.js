...
var FlexmonsterToolbar = function(pivotContainerId, pivot, isHTML5, width, labels) {
	...
	// Options tab
	dataProvider.push({ title: Labels.OPTIONS, id: "fm-tab-options", handler: "optionsHandler", flat: false });

	// Styles tab
	dataProvider.push({ title: Labels.STYLES, id: "fm-tab-styles", html5: false,
		menu: [
			{ title: Labels.STYLES_BLUEBERRY, id: "fm-tab-styles-blueberry", handler: "stylesHandler", args: "blueberry" },
			{ title: Labels.STYLES_ASPHALT, id: "fm-tab-styles-asphalt", handler: "stylesHandler", args: "asphalt" },
			{ title: Labels.STYLES_NAVY, id: "fm-tab-styles-navy", handler: "stylesHandler", args: "navy" },
			{ title: Labels.STYLES_EMERALD, id: "fm-tab-styles-emerald", handler: "stylesHandler", args: "emerald" },
			{ title: Labels.STYLES_COCOA, id: "fm-tab-styles-cocoa", handler: "stylesHandler", args: "cocoa" },
			{ title: Labels.STYLES_RASPBERRY, id: "fm-tab-styles-raspberry", handler: "stylesHandler", args: "raspberry" }
		]
	});
	...
	// Zoom tab
	dataProvider.push({ title: Labels.ZOOM, id: "fm-tab-zoom", html5: false,
		menu: [
			{ title: "50%", handler: "zoomHandler", args: 0.5 },
			{ title: "75%", handler: "zoomHandler", args: 0.75 },
			{ title: "100%", handler: "zoomHandler", args: 1 },
			{ title: "125%", handler: "zoomHandler", args: 1.25 },
			{ title: "150%", handler: "zoomHandler", args: 1.5 },
			{ title: "200%", handler: "zoomHandler", args: 2 },
		]
	});
	...
	self.connectLocalCSVHandler = function() {
		self.pivot.connectTo({ dataSourceType: "csv", browseForFile: true, tableType: (isFlatTable ? "flat" : "pivot") });
	}
	self.connectLocalOCSVHandler = function() {
		self.pivot.connectTo({ dataSourceType: "ocsv", browseForFile: true, tableType: (isFlatTable ? "flat" : "pivot") });
	}
	...
	// Styles tab
	self.stylesHandler = function(style) {
		self.setPivotStyle(style);
	}
	...
	// Zoom tab
	self.zoomHandler = function(value) {
		self.pivot.zoomTo(value);
	}
	...
	self.showConnectToRemoteCSVDialog = function() {
		var applyHandler = function() {
			if (textInput.value.length > 0) {
				self.pivot.connectTo({ filename: textInput.value, dataSourceType: "csv", tableType: (isFlatTable ? "flat" : "pivot") });
			}
		}
	...
	self.showConnectToOLAPDialog = function() {
		var onConnectBtnClick = function() {
			...
			self.pivot.getXMLADataSources(proxyUrlInput.value, 
                ...
                document.getElementById("fm-credentials-checkbox").checked ? document.getElementById("fm-username-input").value : null,
                document.getElementById("fm-credentials-checkbox").checked ? document.getElementById("fm-password-input").value : null);
		};
		...
		var okHandler = function() {
	    	...
	    	pivot.connectTo({
				...
				username: document.getElementById("fm-credentials-checkbox").checked ? document.getElementById("fm-username-input").value : null,
				password: document.getElementById("fm-credentials-checkbox").checked ? document.getElementById("fm-password-input").value : null
			});
		};
		var fillList = function(list, dataProvider, prompt) {
			list.innerHTML = "";
			var optionsHTML = '<option value="" class="placeholder" disabled selected>'+prompt+'</option>';
			for (var i = 0; i < dataProvider.length; i++) {
    			optionsHTML += '<option value="' + dataProvider[i] + '">' + dataProvider[i] + '</option>';
    		}
	...
	self.showFormatCellsDialog = function() {
        ...
        textAlignDropDown.innerHTML = '<option value="left">' + Labels.ALIGN_LEFT + '</option>'
        	+ '<option value="right">' + Labels.ALIGN_RIGHT + '</option>';
        var tr = document.createElement("tr");
        var td = document.createElement("td");
        ...
        thousandsSepDropDown.innerHTML = '<option value="">' + Labels.NONE + '</option>'
        	+ '<option value=" ">' + Labels.SPACE + '</option><option value=",">,</option>';
        var tr = document.createElement("tr");
        var td = document.createElement("td");
        ...
        decimalSepDropDown.innerHTML = '<option value=".">.</option><option value=",">,</option>';
        var tr = document.createElement("tr");
        var td = document.createElement("td");
        ...
        for (var i = 0; i < 11; i++) {
        	...
        	option.value = (i - 1);
        	option.text = (i === 0 ? Labels.NONE : (i - 1));
        	decimalPlacesDropDown.appendChild(option);
        }
        var tr = document.createElement("tr");
        var td = document.createElement("td");
        ...
        currencyAlignDropDown.innerHTML = '<option value="left">' + Labels.ALIGN_LEFT + '</option>'
        	+ '<option value="right">' + Labels.ALIGN_RIGHT + '</option>';
		var tr = document.createElement("tr");
        var td = document.createElement("td");
        ...
        for (var i = 0; i < measures.length; i++) {
        	var option = document.createElement("option");
        	option.value = measures[i].uniqueName;
        	option.text = measures[i].caption;
    ...
    self.createConditionalFormattingItem = function (data, allConditions) {
		...
		var fillConditionsDropDown = function(selectedCondition) {
	        for (var i = 0; i < DEFAULT_CONDITIONS.length; i++) {
	        	var option = document.createElement("option");
	        	option.value = DEFAULT_CONDITIONS[i].sign;
	        	option.text = DEFAULT_CONDITIONS[i].label;
	    ...
	    var fillFontFamiliesDropDown = function(selectedFont) {
		    for (var i = 0; i < DEFAULT_FONTS.length; i++) {
		    	var option = document.createElement("option");
	        	option.value = DEFAULT_FONTS[i];
	        	option.text = DEFAULT_FONTS[i];
		        fontFamiliesDropDown.appendChild(option);
		    }
		...
		var fillFontSizesDropDown = function(selectedFontSize) {
			for (var i = 0; i < DEFAULT_FONT_SIZES.length; i++) {
				var option = document.createElement("option");
	        	option.value = parseInt(DEFAULT_FONT_SIZES[i]);
	        	option.text = DEFAULT_FONT_SIZES[i];
            	fontSizesDropDown.appendChild(option);
	        }
	...
	self.showOptionsDialog = function() {
        ...
        content.appendChild(grandTotalsForm);
        content.appendChild(subTotalsForm);
	...
	// Styles
	self.setPivotStyle = function(styleName) {
		var style = {};
		switch (styleName) {
			case 'cocoa':
				style.backgroundColor = "#FFFFFF";
				style.backgroundAlpha = 1;
				style.borderColor = "#77604a";
				style.color = "#000";
				style.fontSize = 11;
				style.fontFamily  = "Lucida Sans Unicode"
				self.pivot.setStyle(style);
				break;
			case 'emerald':
				style.backgroundColor = "#FFFFFF";
				style.backgroundAlpha = 1;
				style.borderColor = "#4c7e4e";
				style.color = "#000";
				style.fontSize = 11;
				style.fontFamily  = "Lucida Sans Unicode"
				self.pivot.setStyle(style);
	...
	self.showDropdown = function() {
		var menu = this.getElementsByClassName("fm-dropdown")[0];
		if (menu) {
	...
	var PopUpManager = {
		...
		addPopUp: function(popup) {
			...
			popup.style.left = Math.round(((containerRect.right - containerRect.left) - (popupRect.right - popupRect.left)) / 2) + "px";
			popup.style.top = Math.round(((containerRect.bottom - containerRect.top) - (popupRect.bottom - popupRect.top)) / 2) + "px";
		},
